...
class SchwabClientDB:
    ...
    async def _refresh(self):
        ...
        # Always open a fresh session for the DB write — never reuse a possibly-closed one
    ...
    async def get_orders(self, account_hash: str, days_back: int = 60) -> list:
        """
        Schwab allows at most 60 days per request.
        Chunk into 60-day windows and deduplicate by orderId.
        """
        MAX_CHUNK  = 60
        all_orders: list = []
        seen_ids: set    = set()
...
async def get_schwab_client(user_id: str, db: AsyncSession) -> "SchwabClientDB":
    ...
    return SchwabClientDB(
        ...
        expiry=token.expiry,
        user_id=user_id,
        db=None,  # ws.py closes its session before polling; _refresh opens its own
    )
